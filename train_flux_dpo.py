...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to existing LoRA weights to load for training. If provided, will not create new LoRA layers.",
    )
    parser.add_argument(
        "--lora_target_modules",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Start training from this percentage of the diffusion timesteps (0.0 to 1.0).",
    )
    parser.add_argument(
        "--to_prc_timestep",
        type=float,
        default=1.0,
        help="Train until this percentage of the diffusion timesteps (0.0 to 1.0).",
    )
    parser.add_argument(
        "--tracker_name",
        type=str,
    ...
    if args.from_prc_timestep < 0.0 or args.from_prc_timestep > 1.0:
        raise ValueError("from_prc_timestep must be between 0.0 and 1.0")
    if args.to_prc_timestep < 0.0 or args.to_prc_timestep > 1.0:
        raise ValueError("to_prc_timestep must be between 0.0 and 1.0")
    if args.from_prc_timestep >= args.to_prc_timestep:
        raise ValueError("from_prc_timestep must be less than to_prc_timestep")
...
def main(args):
    ...
    if args.load_lora:
        logger.info(f"Loading existing LoRA weights from {args.load_lora}")
        # Assuming load_lora_adapter is a method on flux_transformer or needs a helper
        # This might need adjustment based on how LoRA weights are loaded in your setup.
        # For now, placeholder for actual loading logic:
        # flux_transformer.load_adapter(args.load_lora, adapter_name="default") 
        # Or if it's a direct state dict load:
        # state_dict = torch.load(args.load_lora) # Or however you load the state dict
        # flux_transformer.load_state_dict(state_dict, strict=False) 
        # The diffusers library might have a specific utility for this with PEFT models.
        # Let's assume a method like load_lora_weights or similar exists or will be added:
        # For PEFT models, it's usually `load_adapter`
        from peft import PeftModel
        if isinstance(flux_transformer, PeftModel):
            flux_transformer.load_adapter(args.load_lora, adapter_name="default")
        else:
            logger.warning("flux_transformer is not a PeftModel, direct LoRA loading might require custom implementation or state_dict loading.")
            # As a fallback, if it's not a PeftModel but we expect LoRA, this is problematic.
            # Diffusers' save_lora_weights and lora_state_dict suggest integration with PEFT.
            # If flux_transformer is not already a PeftModel after add_adapter, 
            # then the loading mechanism needs to align with how `add_adapter` integrates LoRA.
            # For now, this will log a warning if not a PeftModel.
            # A more robust solution would involve checking how `add_adapter` makes it a PEFT model.
            # Or using a specific diffusers utility if available.
            # For the purpose of this edit, we'll assume `load_adapter` is the intended PEFT method.

        logger.info(f"Successfully loaded LoRA weights from {args.load_lora}")

    else:
        logger.info("Creating new LoRA layers (as per add_adapter call above).")
    ...
    logger.info(f"  Using LoRA: rank={args.rank}, alpha={args.rank}")
    logger.info(f"  LoRA source: {'Loaded from ' + args.load_lora if args.load_lora else 'Newly initialized'}")
        ...
        for step_val, batch_dict in enumerate(train_dataloader_obj):
            with accelerator.accumulate(flux_transformer):
                ...
                timesteps_indices_val = torch.randint(
                    min_timestep, max_timestep + 1, (bsz_pairs_val,), 
