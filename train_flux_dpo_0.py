...
# from packaging import version # Not strictly needed if not checking xformers version
from peft import LoraConfig #, set_peft_model_state_dict # For loading if needed
...
from diffusers.utils import check_min_version, convert_unet_state_dict_to_peft # convert_unet for peft loading if keys match
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('training.log')  # This saves to a file as well
    ]
)
...
def log_validation(args, flux_transformer_model, vae_model, text_encoder_models, tokenizer_instances, accelerator_obj, weight_dtype_val, current_epoch, is_final_validation_run=False):
    logger.info(f"Running validation... \n Generating images with prompts:\n {VALIDATION_PROMPTS}.")

    # Text encoders are not trained, so they are not wrapped by accelerator.
    # They should already be on the correct device and dtype from main setup.
    ...
    if is_final_validation_run:
        ...
        pipeline = FluxPipeline.from_pretrained(
            ...
            torch_dtype=weight_dtype_val, # For text_encoders and transformer if loaded from scratch
            cache_dir=args.cache_dir,
        )
        # Ensure consistent components are used if they were passed around (e.g., specific VAE)
        pipeline.vae = vae_model
        pipeline.text_encoder = text_encoder_one_val
        pipeline.text_encoder_2 = text_encoder_two_val
        pipeline.load_lora_weights(args.output_dir) # Default name pytorch_lora_weights.safetensors
    else:
        # During training, use the LoRA-adapted transformer (unwrapped)
        # and the existing (frozen) VAE and text encoders
        pipeline = FluxPipeline.from_pretrained(
            args.pretrained_model_name_or_path, # Base for components not explicitly passed
            transformer=accelerator_obj.unwrap_model(flux_transformer_model),
            vae=vae_model,
            text_encoder=text_encoder_one_val, # Pass directly, not wrapped
            text_encoder_2=text_encoder_two_val, # Pass directly, not wrapped
            revision=args.revision,
            variant=args.variant,
            torch_dtype=weight_dtype_val, # This will be applied to models loaded by from_pretrained
            cache_dir=args.cache_dir,
        )
    ...
    for prompt_text in VALIDATION_PROMPTS:
        with context:
            # The pipeline's tokenizer attributes should be set if text_encoders are passed.
            # If not, ensure tokenizers are also passed to encode_prompt if needed.
            # For FLUX, pipeline.encode_prompt uses pipeline.tokenizer and pipeline.tokenizer_2
            # Let's ensure these are set if we are passing custom text_encoders
            pipeline.tokenizer = tokenizer_instances[0]
            pipeline.tokenizer_2 = tokenizer_instances[1]
    ...
    if is_final_validation_run:
        ...
        no_lora_pipeline.tokenizer = tokenizer_instances[0]
        no_lora_pipeline.tokenizer_2 = tokenizer_instances[1]
...
def parse_args(input_args=None):
    ...
    # ... (rest of parse_args is identical to the previous version)
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="flux-dpo-lora-transformer-only", # Clarified
    ...
    parser.add_argument(
        "--enable_xformers_memory_efficient_attention", action="store_true", help="Whether or not to use xformers (if available in FLUX transformer)."
    )
    parser.add_argument(
        "--rank",
        type=int,
...
def encode_prompt_flux(text_encoder_list, prompt_text_inputs_list, device_val, num_repeats_val=1):
    ...
    # Note: The exact structure/meaning of text_ids can vary.
    # For basic text conditioning, zeros might be okay. The FluxPipeline._prepare_text_ids
    # also generates zeros if not provided.
    # Changed to 2D tensor to address deprecation warning: (bsz, seq_len, 3) -> (seq_len, 3)
...
def main(args):
    ...
    try:
        ...
        logger.info(f"Dataset type: {type(train_dataset_obj)}")
        ...
        if hasattr(train_dataset_obj, 'column_names'):
            ...
            if missing_columns:
                raise ValueError(
                    ...
                    "Please ensure your dataset contains all required columns: "
                    "'jpg_0', 'jpg_1' (image bytes), 'label_0' (0 if jpg_0 is preferred), and 'caption'"
                )
        else:
            raise ValueError("Dataset does not have a valid column structure")
            
        # Try accessing first item to validate data
        first_item = train_dataset_obj[0]
        logger.info("First dataset item structure:")
        logger.info(f"Keys: {list(first_item.keys())}")
        
    except Exception as e:
        logger.error(f"Error loading or validating dataset: {str(e)}")
        logger.error("Please ensure your dataset is properly formatted and contains the required columns")
        raise
    ...
    # Keep a list of tokenizers for easy passing to validation
    ...
    # Keep a list of text_encoders for easy passing to validation
    ...
    flux_transformer.requires_grad_(False) # Will be selectively un-frozen by LoRA
    ...
    vae.to(accelerator.device, dtype=torch.float32) # VAE usually fp32
    text_encoder_one.to(accelerator.device, dtype=weight_dtype)
    text_encoder_two.to(accelerator.device, dtype=weight_dtype)
    # flux_transformer will be moved by accelerator.prepare()
    # but cast its non-LoRA parts to weight_dtype before adding adapter
    ...
    flux_transformer.add_adapter(transformer_lora_config) # This makes LoRA params require_grad=True

    if args.mixed_precision == "fp16": # Ensure LoRA params are fp32 for training if base is fp16
        for param_name, param_val in flux_transformer.named_parameters():
            if param_val.requires_grad: # Only upcast LoRA parameters
                param_val.data = param_val.to(torch.float32)


    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available() and hasattr(flux_transformer, 'enable_xformers_memory_efficient_attention'):
            try:
        ...
        elif not hasattr(flux_transformer, 'enable_xformers_memory_efficient_attention'):
            logger.warning("FluxTransformer2DModel does not have 'enable_xformers_memory_efficient_attention' method.")
        else: logger.warning("XFormers not available.")
    ...
    def save_model_hook(models_list, weights_list, output_dir_hook):
        if accelerator.is_main_process:
            ...
            # Assuming models_list contains only the flux_transformer (the one being trained)
            if len(models_list) == 1 and isinstance(models_list[0], type(unwrap_model_for_saving_hook(flux_transformer))):
                ...
                weights_list.pop() # Pop the corresponding weight
            else:
                # This case should ideally not happen if only flux_transformer is prepared
                # Fallback or error if multiple unexpected models are here
                for model_hook in models_list:
                    if isinstance(model_hook, type(unwrap_model_for_saving_hook(flux_transformer))):
                        ...
                        # How to pop the correct weight? This needs careful index matching if weights_list has multiple items.
                        # For simplicity, assuming one-to-one mapping if this branch is hit.
                        logger.warning("save_model_hook: Multiple models found, attempting to save FluxTransformer LoRA.")
                        if weights_list: weights_list.pop(0) # Risky, assumes order
                        break 
    ...
    def load_model_hook(models_list, input_dir_hook):
        ...
        # Assuming models_list contains only the flux_transformer
        if len(models_list) == 1 and isinstance(models_list[0], type(unwrap_model_for_saving_hook(flux_transformer))):
            transformer_hook = models_list.pop()
        else:
            # Fallback or error
            for i in range(len(models_list) -1, -1, -1): # Iterate backwards to pop safely
        ...
        # For PEFT models, state_dict is often flat with prefixes.
        # `convert_unet_state_dict_to_peft` might work if naming conventions are similar enough.
        # Or, filter manually if needed.
        transformer_peft_state_dict = {}
        if "transformer" in lora_state_dict_hook: # If keys are nested under 'transformer'
            transformer_peft_state_dict = convert_unet_state_dict_to_peft(lora_state_dict_hook["transformer"])
        else: # If keys are flat like "transformer.lora_A..."
        ...
        if transformer_hook is not None and transformer_peft_state_dict:
            from peft import set_peft_model_state_dict # explicit import
            ...
            # Ensure LoRA params are in correct dtype after loading if mixed_precision was used
            if args.mixed_precision == "fp16":
                for param_name_load, param_val_load in transformer_hook.named_parameters():
                    if param_val_load.requires_grad:
                        param_val_load.data = param_val_load.to(torch.float32)
    ...
    train_flip_transform = transforms.RandomHorizontalFlip(p=1.0) # Applied with 0.5 prob below
    ...
    def preprocess_train_fn(examples_dict):
        ...
        for img_w_item, img_l_item in zip(images_w_pil, images_l_pil):
            ...
            if not args.no_hflip and random.random() < 0.5: img_w_p = train_flip_transform(img_w_p)
            ...
            if not args.no_hflip and random.random() < 0.5: img_l_p = train_flip_transform(img_l_p)
    ...
    # Only flux_transformer (with LoRA) is optimized. VAE and Text Encoders are frozen.
    ...
    def get_sigmas_fn(timesteps_in, n_dim_in=4, dtype_in=torch.float32, device_in=None):
        ...
        while len(sigma_out.shape) < n_dim_in: sigma_out = sigma_out.unsqueeze(-1)
        return sigma_out
    ...
    for epoch_val in range(first_epoch_val, args.num_train_epochs):
        for step_val, batch_dict in enumerate(train_dataloader_obj):
            with accelerator.accumulate(flux_transformer):
                pixel_values_in = batch_dict["pixel_values"].to(dtype=vae.dtype) # To VAE's dtype (fp32)
                latents_list_val = []
                with torch.no_grad(): # VAE is frozen
                    for i_val_vae in range(0, pixel_values_in.shape[0], args.vae_encode_batch_size):
                        latents_list_val.append(
                            vae.encode(pixel_values_in[i_val_vae : i_val_vae + args.vae_encode_batch_size]).latent_dist.sample()
                        )
                latents_in = torch.cat(latents_list_val, dim=0)
                latents_in = (latents_in - vae_shift) * vae_scale
                latents_in = latents_in.to(dtype=weight_dtype) # To training dtype
                ...
                timesteps_indices_val = torch.randint(
                    0, noise_scheduler.config.num_train_timesteps, (bsz_pairs_val,), device=latents_in.device, dtype=torch.long
                )
                # Move timesteps to the same device as indices before indexing
                ...
                sigmas_val = get_sigmas_fn(timesteps_val, n_dim_in=latents_in.ndim, dtype_in=latents_in.dtype, device_in=latents_in.device)
                ...
                guidance_input = torch.full(
                    (timesteps_val.shape[0],), # Batch size for the transformer input
                    unconditional_guidance_scale,
                    device=accelerator.device,
                    dtype=weight_dtype 
                )

                # Policy Model - Fixed the forward call
                ...
                # Reference Model - Disable LoRA adapters to get base model behavior
                with torch.no_grad():
                    # Temporarily disable LoRA adapters to get reference model predictions
                    ...
                    # Re-enable LoRA adapters for subsequent training
                ...
                optimizer.zero_grad(set_to_none=True) # set_to_none for potential minor speedup
                ...
                if accelerator.is_main_process:
                    if global_step_val % args.checkpointing_steps == 0:
                        if args.checkpoints_total_limit is not None:
                            # Checkpoint limiting logic (same as before)
                            ...
                            if len(ckpts) >= args.checkpoints_total_limit:
                                ...
                                for rm_ckpt in removing_ckpts: shutil.rmtree(os.path.join(args.output_dir, rm_ckpt))
                    ...
                    if args.run_validation and global_step_val % args.validation_steps == 0:
                        log_validation(
                            ...
                            accelerator, weight_dtype, epoch_val
                        )
                        flux_transformer.train() # Back to train mode

                logs_dict = {
                    "loss": loss_val.detach().item(), "policy_mse": raw_policy_mse_log.detach().item(),
                    "ref_mse": raw_ref_mse_log.detach().item(), "accuracy": implicit_acc_log.detach().item(),
                    "lr": lr_scheduler_obj.get_last_lr()[0],
                }
                progress_bar_obj.set_postfix(**logs_dict)
                accelerator.log(logs_dict, step=global_step_val)

                if global_step_val >= args.max_train_steps: break
        if global_step_val >= args.max_train_steps: break

    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        flux_transformer_final_save = accelerator.unwrap_model(flux_transformer)
        # Save in float32 for wider compatibility, or training dtype to save space
        ...
        if args.run_validation:
            log_validation(
                args, flux_transformer, vae, all_text_encoders, all_tokenizers,
                accelerator, weight_dtype, epoch_val, is_final_validation_run=True
            )

        if args.push_to_hub:
            logger.info("Pushing model to hub...")
            # Potentially create a more detailed model card here
            upload_folder(
